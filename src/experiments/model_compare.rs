...
use crate::parameters::quantity::{Force, Length, Quantity, Stress, Time};
...
fn group_layout(
    ...
    let centroid = V2D {
        x: char_quants.normalize(&Length((group_ix as f64) * 40.0).micro().g()),
        ...
        .filter_map(|(group_ix, &num_cells)| {
            if num_cells > 0 {
                Some(CellGroup {
                    ...
                    parameters: gen_raw_params(group_ix, rng, randomization)
                        .gen_parameters(cq),
                })
            } else {
                None
...
pub fn generate(
    ...
    let world_parameters = raw_world_parameters(&char_quants, coa_mag, cil_mag)
        .refine(&char_quants);
    let cell_groups =
        cell_groups(&mut rng, randomization, &char_quants, num_cells_per_group);
    Experiment {
