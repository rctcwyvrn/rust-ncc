...
impl DistributionScheme {
    ...
    fn scaled_unitize(frac: f64, mut distrib: [f64; NVERTS]) -> [f64; NVERTS] {
        let sum: f64 = distrib.iter().sum();
        distrib.iter_mut().for_each(|e| *e = *e * frac / sum);
        distrib
    }

    fn gen_random(rng: &mut Pcg32, frac: f64) -> [f64; NVERTS] {
    ...
    fn gen_specific(frac: f64, marked_verts: &[bool; NVERTS]) -> [f64; NVERTS] {
    ...
    pub fn generate(&self, rng: &mut Pcg32) -> [f64; NVERTS] {
        match &self.ty {
            DistributionType::Random => Self::gen_random(rng, self.frac),
...
impl RacRandState {
    ...
    pub fn update(
        ...
        if tstep == self.next_update && self.enabled {
            let next_update = tstep + self.distrib.sample(rng).floor() as u32;
