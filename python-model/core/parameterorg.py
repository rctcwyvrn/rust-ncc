...
user_rho_gtpase_biochemistry_parameters = {
    ...
    "tension_mediated_rac_inhibition_half_strain": [0.01, 0.99],
    "tension_mediated_rac_inhibition_magnitude": [1.0, 100.0],
    "strain_calculation_type": None,
    "init_rgtpase_cytosol_frac": [0, 1],
    "hill_exponent": 3,
    "kgtp_rac_multiplier": [1, 500],
    "max_coa_signal": [-1, 10],
    "H_total": [0.5e6, 1.5e6],
    "diffusion_const": [2e-14, 4.5e-13],
...
user_model_run_parameters = {
    "skip_dynamics": None,
    "biased_rgtpase_distrib_defn": None,
    "num_nodes": [3, 100],
}

all_user_parameters_with_justifications = {}
for parameter_dict in [
...
def make_cell_group_parameter_dict(justify_parameters, user_parameter_dict):
    ...
    assert (
        user_parameter_dict["init_rgtpase_cytosol_frac"]
        + user_parameter_dict["init_rgtpase_membrane_inactive_frac"]
        + user_parameter_dict["init_rgtpase_membrane_active_frac"]
        == 1
    )

    cell_parameter_dict["init_rgtpase_cytosol_frac"] = user_parameter_dict[
        "init_rgtpase_cytosol_frac"
    ]
    cell_parameter_dict["init_rgtpase_membrane_inactive_frac"] = user_parameter_dict[
        "init_rgtpase_membrane_inactive_frac"
    ]
    cell_parameter_dict["init_rgtpase_membrane_active_frac"] = user_parameter_dict[
        "init_rgtpase_membrane_active_frac"
    ]

    # --------------
    cell_parameter_dict["kgtp_rac_baseline"] = (
        kgtp_rac_unmodified * user_parameter_dict["kgtp_rac_multiplier"]
    )  # per second
    cell_parameter_dict["kdgtp_rac_baseline"] = (
        kdgtp_rac_unmodified * user_parameter_dict["kdgtp_rac_multiplier"]
    )  # per second
    # --------------
    cell_parameter_dict["kgtp_rho_baseline"] = (
        kgtp_rho_unmodified * user_parameter_dict["kgtp_rho_multiplier"]
    )  # per second
    cell_parameter_dict["kdgtp_rho_baseline"] = (
        kdgtp_rho_unmodified * user_parameter_dict["kdgtp_rho_multiplier"]
    )  # per second
    # --------------
    ...
    cell_parameter_dict["kdgtp_rho_mediated_rac_inhib_baseline"] = (
        ...
        * user_parameter_dict["kdgtp_rho_mediated_rac_inhib_multiplier"]
    )  # per second
    cell_parameter_dict["kdgtp_rac_mediated_rho_inhib_baseline"] = (
        kdgtp_rho_unmodified
        * user_parameter_dict["kdgtp_rac_mediated_rho_inhib_multiplier"]
    )  # per second
    # --------------
    kgdi = 0.15
    kdgdi = 0.02
    cell_parameter_dict["kgdi_rac"] = (
        kgdi * user_parameter_dict["kgdi_multiplier"]
    )  # per second
    cell_parameter_dict["kdgdi_rac"] = (
        kdgdi * user_parameter_dict["kdgdi_multiplier"]
    )  # per second
    # --------------
    cell_parameter_dict["kgdi_rho"] = (
        kgdi * user_parameter_dict["kgdi_multiplier"]
    )  # per second
    cell_parameter_dict["kdgdi_rho"] = (
        kdgdi * user_parameter_dict["kdgdi_multiplier"]
    )  # per second
    # --------------
    cell_parameter_dict["threshold_rac_activity"] = (
        user_parameter_dict["threshold_rac_activity_multiplier"] * C_total
    )
    cell_parameter_dict["threshold_rho_activity"] = (
        user_parameter_dict["threshold_rho_activity_multiplier"] * H_total
    )
    # --------------
    cell_parameter_dict["diffusion_const_active"] = user_parameter_dict[
        "diffusion_const"
    ]  # micrometers squared per second
    cell_parameter_dict["diffusion_const_inactive"] = user_parameter_dict[
        "diffusion_const"
    ]  # micrometers squared per second
    # --------------
    ...
    cell_parameter_dict[
        "tension_mediated_rac_inhibition_half_strain"
    ] = user_parameter_dict["tension_mediated_rac_inhibition_half_strain"]
    cell_parameter_dict[
        "tension_mediated_rac_inhibition_magnitude"
    ] = user_parameter_dict["tension_mediated_rac_inhibition_magnitude"]
    cell_parameter_dict["strain_calculation_type"] = user_parameter_dict[
        "strain_calculation_type"
    ]
    cell_parameter_dict["max_coa_signal"] = user_parameter_dict["max_coa_signal"]
    cell_parameter_dict["coa_sensing_dist_at_value"] = user_parameter_dict[
        "coa_sensing_dist_at_value"
    ]
    cell_parameter_dict["coa_sensing_value_at_dist"] = user_parameter_dict[
        "coa_sensing_value_at_dist"
    ]
    ...
    cell_parameter_dict["stiffness_cytoplasmic"] = user_parameter_dict[
        "stiffness_cytoplasmic"
    ]
    cell_parameter_dict["length_edge_resting"] = length_edge_resting
    cell_parameter_dict["max_force_rac"] = (
        user_parameter_dict["max_force_rac"] * length_edge_resting * 200e-9
    )
    cell_parameter_dict["max_force_rho"] = (
        user_parameter_dict["force_rho_multiplier"]
        * cell_parameter_dict["max_force_rac"]
    )
    cell_parameter_dict["max_protrusive_nodal_velocity"] = (
        cell_parameter_dict["max_force_rac"] / cell_parameter_dict["eta"]
    )
    cell_parameter_dict["threshold_force_rac_activity"] = (
        user_parameter_dict["threshold_rac_activity_multiplier"] * C_total
    )
    cell_parameter_dict["threshold_force_rho_activity"] = (
        user_parameter_dict["threshold_rho_activity_multiplier"] * H_total
    )
    cell_parameter_dict["force_adh_const"] = user_parameter_dict["force_adh_const"]
    # --------------
    cell_parameter_dict["closeness_dist_squared_criteria"] = user_parameter_dict[
        "closeness_dist_squared_criteria"
    ]
    cell_parameter_dict["interaction_factor_migr_bdry_contact"] = user_parameter_dict[
        "interaction_factor_migr_bdry_contact"
    ]
    cell_parameter_dict[
        "interaction_factors_intercellular_contact_per_celltype"
    ...
    cell_parameter_dict["randomization_time_mean"] = user_parameter_dict[
        "randomization_time_mean"
    ]
    cell_parameter_dict["randomization_time_variance_factor"] = user_parameter_dict[
        "randomization_time_variance_factor"
    ]
    cell_parameter_dict["randomization_magnitude"] = user_parameter_dict[
        "randomization_magnitude"
    ]
    cell_parameter_dict["randomization_node_percentage"] = user_parameter_dict[
        "randomization_node_percentage"
    ]
    cell_parameter_dict["randomization_type"] = user_parameter_dict[
        "randomization_type"
    ]
...
def expand_interaction_factors_intercellular_contact_per_celltype_array(
    ...
    if num_defns != num_cell_groups:
        raise Exception(
            "Number of cell groups does not equal number of keys in interaction_factors_intercellular_contact_per_celltype_defn."
        )
...
def expand_interaction_factors_coa_per_celltype_array(
    ...
    interaction_factors_coa_per_celltype_defn = this_cell_group_defn[
        "interaction_factors_coa_per_celltype"
    ]
...
def make_environment_given_user_cell_group_defns(
    ...
    for cell_group_defn_index, user_cell_group_defn in enumerate(user_cell_group_defns):
        ...
        user_cell_group_parameter_dict[
            ...
            num_cell_groups, user_cell_group_defns, user_cell_group_defn
        )
        user_cell_group_parameter_dict[
            "interaction_factors_coa_per_celltype"
        ...
        cell_group_parameter_dict = make_cell_group_parameter_dict(
